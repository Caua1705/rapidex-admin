...
import type { OrderPrintJobs, PrintJob } from '../api/types';
...
describe('ComandaDoPedido', () => {
  ...
  it('não chama o backend enquanto o lojista não pede', () => {
    render(<ComandaDoPedido orderId="ped-1" paymentStatus="paid" />);
  ...
  it('fecha e esquece a comanda quando o pedido troca', async () => {
    const { rerender } = render(<ComandaDoPedido orderId="ped-1" paymentStatus="paid" />);
    await userEvent.click(screen.getByTestId('comanda-abrir'));
    await screen.findByTestId('comanda-via');

    rerender(<ComandaDoPedido orderId="ped-2" paymentStatus="paid" />);
